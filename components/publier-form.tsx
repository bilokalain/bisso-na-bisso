...
type Props = { type: AnnonceType };

export function PublierForm({ type }: Props) {
    ...
    <form action={formAction} className="space-y-10">
      ...
      <Section title="Ton annonce" subtitle="Ce que les gens verront d'abord.">
        ...
        >
          <input
            ...
            maxLength={120}
            className={inputCls}
            placeholder={placeholderByType[type].titre}
          />
        </Field>
        ...
        >
          <textarea
            ...
            placeholder={placeholderByType[type].description}
          />
        </Field>
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          ...
          <Field
            label={type === "colis" ? "Prix total (EUR)" : "Prix (EUR)"}
            hint={
              type === "repetiteur" ? "Par heure." : "Laisse vide si variable."
            }
            error={err.prix?.[0]}
          >
            <input
      ...
      <div className="flex flex-col items-stretch gap-3 border-t border-sand pt-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          type="submit"
          disabled={pending}
          className={`inline-flex items-center justify-center gap-2 rounded-full px-6 py-3 text-base font-medium text-ivory shadow-card transition active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60 ${vertical.accentBg} hover:brightness-95`}
        >
